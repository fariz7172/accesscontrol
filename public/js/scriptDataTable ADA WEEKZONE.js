// Function to convert card number
function convertCardNumber(cardNo, machineType) {
    ...
    if (isNaN(cardNumber) || cardNumber < 0) {
        ...
        return '00000:00000';  // Default fallback to zero-padded formatted
    }

    let byte6 = (cardNumber >> 16) & 0xFF;
    let byte7 = (cardNumber >> 8) & 0xFF;
    let byte8 = cardNumber & 0xFF;
    let byte5 = 0x21;  // Fixed like in PHP (for word1 = 8481 base, padded 08481)

    // Combine bytes like PHP inverse logic
...
$(document).ready(function () {
    ...
    // Check/Uncheck all user checkboxes
    ...
    $('#insertDataButton').on('click', async function () {
        ...
        // Get the selected weekzone_id from the dropdown
        const weekzoneId = $('#weekzone_id').val();
        if (!weekzoneId) {
            Swal.fire({
                title: 'Error',
                text: 'Please select a weekzone!',
                icon: 'error'
            });
        ...
        async function processUsers() {
            const users = $('.user-checkbox:checked').map(function () {
                ...
                const beginDate = row.find('td:nth-child(4)').text().trim(); // BEGIN_DATE column
                const endDate = row.find('td:nth-child(5)').text().trim();   // END_DATE column
                return {
            ...
            for (const user of users) {
                ...
                try {
                    await processMachines(user.userId, user.username, user.cardNumber, user.beginDate, user.endDate, imageData, selectedMachines, weekzoneId);
                } catch (err) {
        ...
        async function processMachines(userId, username, cardNumber, beginDate, endDate, imageData, machines, weekzoneId) {
            for (const machine of machines) {
                ...
                let data = {
                    ...
                    fdata: "",
                    weekzone: weekzoneId // Add weekzone to data
                };
                ...
                async function proceedWithData(fdataValue) {
                    ...
                    try {
                        if (machine.type == '1') {
                            await sendDataToSoyalApi(userId, username, processedCardNumber, machine, weekzoneId);
                        } else {
                            await sendDataToApi(data, userId, machine.id, machine.ip);
                        }

                        // If machine.type is 0 or 50, send data to sztimmyUserAccessApiUrl
                        if (machine.type == '0' || machine.type == '50') {
                            // Format dates to Y-m-d H:i:s if not already in that format
                            const formattedBeginDate = formatDateToApi(beginDate);
                            const formattedEndDate = formatDateToApi(endDate);

                            // PUSH DATA TO usersAccess API
                            const accessData = {
                                sn: machine.sn,
                                userid: userId,
                                starttime: formattedBeginDate || '2025-03-01 00:00:00', // Fallback if null
                                endtime: formattedEndDate || '2025-06-26 23:59:00',   // Fallback if null
                                weekzone: weekzoneId // Add weekzone to accessData
                            };
        ...
        // Function to format date to Y-m-d H:i:s
        ...
        async function sendDataToSoyalApi(userId, username, cardNumber, machine, weekzoneId) {
                ...
                try {
                    const response = await $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            timezone: weekzoneId, // Use weekzoneId instead of hardcoded '0'
                    ...
                    if (attempt === 3) {
                        ...
                        Swal.fire({
                            title: 'Error',
                            text: `Failed to send data to Soyal API for user ${userId} after 3 attempts`,
                            icon: 'error'
                        });
                        completedRequests++;
                        updateProgress();
        ...
        async function sendDataToApi(data, userId, machineId, machineIp) {
            const currentTime = moment().tz('Asia/Jakarta').format('YYYY-MM-DD HH:mm:ss');
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    ...
                    if (attempt === 3) {
                        ...
                        const logData = {
                            ...
                            desc: `Failed Insert User ${data.username.substring(0, 100)}`,
                            status: 'Failed'
                        };
                        saveToDeviceLog(logData);
                        completedRequests++;
    ...
    // Handle Delete To Machine button in modal
