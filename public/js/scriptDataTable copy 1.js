...
async function fetchUserDatesAndCreateAccessData(userId, data) {
    try {
        ...
        const accessData = {
            ...
            weekzone: response.weekzone ?? 0  // DEFAULT = 0
        };
...
$(document).ready(function () {
    ...
    $('#insertDataButton').on('click', function () {
        ...
        let allAccessPayloads = []; // Kumpul semua access data
        ...
        async function processUsers() {
            ...
            // Kirim batch useraccess setelah semua register selesai
       ...
       async function processMachines(userId, username, cardNumber, imageData, machines) {
            ...
            // Ambil data tambahan dari backend (Card & Fp)
            let userFpData = null;
            try {
            ...
            for (const machine of machines) {
                // === 1. CARDNUMBER: Selalu dari usersprofile.Card ===
                let processedCardNumber = cardNumber; // cardNumber dari data-usercard di checkbox
                if (machine.type == '1' || machine.type == '3') {
                    processedCardNumber = convertCardNumber(cardNumber, machine.type);
                    console.log(`[CONVERT] Card ${cardNumber} → ${processedCardNumber} (type=${machine.type})`);
                } else {
                    console.log(`[CARD] : ${cardNumber}`);
                }

                // === 2. FDATA: Khusus untuk type=0, ambil dari Fp ===
                let fdataValue = "";
                if (machine.type === '0') {
                    if (userFpData && userFpData !== '0' && userFpData.trim() !== '') {
                        fdataValue = userFpData;
                        console.log(`[FDATA] type=0 → Pakai Fp dari tbl_usersdata: ${fdataValue.substring(0, 50)}...`);
                    } else {
                        fdataValue = "";
                        console.log(`[FDATA] type=0 → Fp kosong → fdata = ""`);
                    }
                } else if (machine.type === '50' && imageData) {
                    fdataValue = imageData;
                    console.log(`[FDATA] type=50 → Pakai foto dari profil`);
                } else {
                    fdataValue = "";
                }
                ...
                // === 4. SIMPAN KE tbl_usersdata (jika bukan 0,1,3,50) ===
                if (!['0', '1', '3', '50'].includes(machine.type)) {
            ...
            return accessPayloads;
                ...
                // Kirim registerface + kumpul access data
                async function sendDataToSztimmyRegisterFace(data, userId, machineId, machineIp) {
                    ...
                    for (let attempt = 1; attempt <= 3; attempt++) {
                        try {
                            ...
                            return accessData; // Kembalikan untuk batch
        ...
        function saveToUserDataModel(userId, sn, type) {
            ...
            // Hanya simpan info penting, JANGAN fdata Base64
            $.post('/save-to-user-data', {
                userid: userId,
                sn: sn,
                type: type,
                desc: logDesc
            }).fail(function(xhr) {
                console.error('Gagal simpan log:', xhr.responseText);
            });
        }
