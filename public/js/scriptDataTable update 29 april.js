    ...
    $('#insertDataButton').on('click', function () {
        ...
        async function sendBatchUserAccess(payloads) {
            ...
            try {
                const response = await $.ajax({
                    ...
                    method: 'POST',
                    contentType: 'application/json',
                    data: JSON.stringify(payloads) // ARRAY!
                });

                const logData = {
                    log_date: currentTime,
                    modul: JSON.stringify(payloads),
                    desc: `Success Batch User Access (${payloads.length} records)`,
                    status: 'Successfully'
                };
                saveToDeviceLog(logData);
                ...
                const logData = {
                    log_date: currentTime,
                    modul: JSON.stringify(payloads),
                    desc: `Failed Batch User Access: ${err.responseJSON?.message || err.message}`,
                    status: 'Failed'
                };
                saveToDeviceLog(logData);
            }
        ...
        function saveToUserDataModel(userId, sn, type) {
            $.ajax({
                ...
                method: 'POST',
                contentType: 'application/json',
                data: JSON.stringify({ fid: userId, sn, type, picture: "0", fp: "0" }),
                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }
            });
        }
