...
class userProfileController extends Controller
{
    ...
    public function getDevicesByGroup($deviceGroupId)
    {
        try {
            if ($deviceGroupId === 'all') {
                ...
                // Query directly using the foreign key 'groupid'
                $devices = deviceGateModel::where('groupid', $deviceGroupId)->get();
            }

            return response()->json($devices);
        } catch (\Exception $e) {
            Log::error('Error fetching devices by group: ' . $e->getMessage());
    ...
    public function registerUser(Request $request)
    {
        $data = $request->validate([
            ...
            'card_number'   => 'required|string',               // format: 00093:21611
            'ip_address'    => 'required|ip',
            'node_id'       => 'required|integer',
            'timezone'      => 'required|integer',
            'begin_date'    => 'required|date_format:m-d-Y',    // contoh: 01-01-2025
            'expire_date'   => 'required|date_format:m-d-Y',    // contoh: 09-28-2026
            'begin_time'    => 'required|date_format:H:i',      // contoh: 00:01
            'expire_time'   => 'required|date_format:H:i',      // contoh: 23:59
            'pin'           => 'required|string',
            'user_name'     => 'required|string',
            'type'          => 'required|string',

            // Lift control (opsional, default 255 = semua lantai diizinkan)
        ...
        // Default lift = 255 (semua lantai diizinkan) jika tidak dikirim
        $lift1 = $data['lift1'] ?? 255;
        $lift2 = $data['lift2'] ?? 255;
        $lift3 = $data['lift3'] ?? 255;
        $lift4 = $data['lift4'] ?? 255;
        ...
        // Parse tanggal & waktu
        $beginDate  = DateTime::createFromFormat('m-d-Y', $data['begin_date']);
        $expireDate = DateTime::createFromFormat('m-d-Y', $data['expire_date']);
        ...
        // Buat paket 41 byte (index 0–40)
        $bytes = array_fill(0, 41, 0);

        $bytes[0]  = 0x7E;                    // STX
        $bytes[1]  = 0x26;                    // Length
        $bytes[2]  = (int)$data['node_id'];   // Node ID
        $bytes[3]  = 0x8B;
        $bytes[4]  = 0x57;
        ...
        // Begin Date (yy MM dd) + Begin Time (HH mm)
        $bytes[24] = (int)$beginDate->format('y');   // tahun 2 digit
        $bytes[25] = (int)$beginDate->format('m');   // bulan
        $bytes[26] = (int)$beginDate->format('d');   // tanggal
        $bytes[27] = (int)$beginTime[0];             // jam mulai
        $bytes[28] = (int)$beginTime[1];             // menit mulai

        // Expire Date (yy MM dd) + Expire Time (HH mm)
        $bytes[29] = (int)$expireDate->format('y');
        $bytes[30] = (int)$expireDate->format('m');
        $bytes[31] = (int)$expireDate->format('d');
        $bytes[32] = (int)$expireTime[0];
        $bytes[33] = (int)$expireTime[1];

        // LIFT CONTROL — Ini yang kamu minta dari kode VB
        $bytes[34] = $lift1;   // Lift 1
        $bytes[35] = $lift2;   // Lift 2
        $bytes[36] = $lift3;   // Lift 3
        $bytes[37] = $lift4;   // Lift 4

        // Hitung LRC (XOR dari byte 2 sampai 37)
        $LRC = 255;
        for ($i = 2; $i <= 37; $i++) {
            $LRC ^= $bytes[$i];
        }
        ...
        // Hitung Checksum (SUM byte 2 sampai 38, lalu modulo 256)
        $sum = 0;
        for ($i = 2; $i <= 38; $i++) {
            $sum += $bytes[$i];
        }
        ...
        if ($response->getStatusCode() === 200) {
            return response()->json([
                ...
                'lift_access' => [$lift1, $lift2, $lift3, $lift4],
            ], 200);
        }

        return response()->json([
            'message' => 'Failed to register user to Soyal controller',
            'error'   => $response->getData()->message ?? 'Connection failed'
        ], 400);
    }
    ...
    public function saveToUserData(Request $request)
    {
        ...
        try {
            // Coba update dulu, jika tidak ada maka insert
            $userData = userDataModel::updateOrCreate(
                [
                    'fid'    => $data['fid'],
                    'Type'   => $data['type'],
                    'FaceNo' => $data['sn'],
                ...
                [
                    'Fp'      => $data['fp'] ?? null,      // null jika tidak ada data baru
                    'Picture' => $data['picture'] ?? null, // null jika tidak ada foto
                ]
            );

            return response()->json(['success' => true, 'message' => 'Data saved/updated']);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function checkUserDataFp($userId, $sn)
    {
        $exists = userDataModel::where('fid', $userId)
            ->where('FaceNo', $sn)
            ->whereNotNull('Fp')
            ->where('Fp', '!=', '')
            ->where('Fp', '!=', '0')
