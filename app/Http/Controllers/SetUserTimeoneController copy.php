...
class SetUserTimeoneController extends Controller
{
    ...
    public function assignWeekzone(Request $request)
    {
        ...
        if (empty($userIds) || empty($deviceIds) || empty($weekzoneIds)) {
            return redirect()->back()->withErrors(['error' => 'Data tidak lengkap.']);
        }
        ...
        try {
            ...
            foreach ($userIds as $userId) {
                ...
                foreach ($deviceIds as $deviceId) {
                    if (!isset($devices[$deviceId])) {
                        $errors[] = "Device ID {$deviceId} tidak valid.";
                        continue;
                    }
            ...
            $message = "Berhasil assign weekzone ke {$successCount} kombinasi user-device.";
    ...
    {
        if (!$url) {
            Log::warning("API URL tidak ditemukan.");
            return false;
        }
        ...
        try {
            ...
            Log::error("Gagal POST ke API: " . $e->getMessage());
            return false;
        }
    }
}
