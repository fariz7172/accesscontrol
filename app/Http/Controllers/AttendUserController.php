            ...
            DB::transaction(function () use ($request, $daysInMonth, $year, $month, $dayToPolaMap, $pattern, $holidays, $sortedUserIds, &$createdCount, &$updatedCount) {
                ...
                for ($day = 1; $day <= $daysInMonth; $day++) {
                    ...
                    foreach ($sortedUserIds as $user_id) {
                        ...
                        // Hitung WorkTime dari tabel shift
                        if ($shift && $shift->Begin_Time && $shift->Out_time && $shift->Break_Time && $shift->Resume_Time) {
                            ...
                            // WorkTime = (Out_time - Begin_Time) - (Resume_Time - Break_Time)
                            $workDuration = $outTime->diffInMinutes($beginTime);
                            $breakDuration = $resumeTime->diffInMinutes($breakTime);
                            $attendanceData['WorkTime'] = max(0, $workDuration - $breakDuration);
                        }
                        ...
                        if (!$isHoliday && !$leaveRecord && $shift && $shift->Start_In && $shift->Begin_Time && $shift->Range_In) {
                            ...
                            if ($logs->isNotEmpty()) {
                                ...
                                if ($logsBetweenStartInAndBeginTime->isNotEmpty()) {
                                    // Ada TM_EVENT antara Start_In dan Begin_Time, ambil yang terkecil (MIN)
                                    ...
                                    // Tidak ada TM_EVENT antara Start_In dan Begin_Time, tetapi ada setelah Begin_Time hingga Range_In, ambil yang terkecil
                                ...
                                if ($shift->Start_Break && $shift->Range_Break && $shift->Break_Time && $shift->Resume_Time) {
                                    ...
                                    // Time_Break: Ambil TM_EVENT terkecil (MIN) antara Break_Time dan Resume_Time
                                    ...
                                    if ($logsBetweenBreakAndResume->isNotEmpty()) {
                                        ...
                                        $attendanceData['Present'] = 1;
                                        $attendanceData['Time_BreakShort'] = 0; // Set ke 0 jika ada log antara Break_Time dan Resume_Time
                                    } else {
                                        ...
                                        if ($logsBetweenStartBreakAndBreakTime->isNotEmpty()) {
                                            ...
                                            $attendanceData['Present'] = 1;
                                            $attendanceData['Time_BreakShort'] = abs($latestLog->diffInMinutes($breakTime));
                                        } else {
                                ...
                                if ($shift->Start_Resume && $shift->Range_Resume && $shift->Break_Time && $shift->Resume_Time) {
                                    ...
                                    if ($logsBetweenBreakAndResume->isNotEmpty()) {
                                        ...
                                        $attendanceData['Present'] = 1;
                                        $attendanceData['Time_ResumeShort'] = $latestLog->gt($resumeTime) ? abs($latestLog->diffInMinutes($resumeTime)) : 0;
                                    } else {
                                        ...
                                        if ($logsBetweenStartResumeAndRangeResume->isNotEmpty()) {
                                            ...
                                            $attendanceData['Present'] = 1;
                                            $attendanceData['Time_ResumeShort'] = $earliestLog->gt($resumeTime) ? abs($earliestLog->diffInMinutes($resumeTime)) : 0;
                                        } else {
                                ...
                                // Proses Time_Out, Time_OutShort, dan TotalOT
                                if ($shift->Out_time && $shift->Start_Out && $shift->Range_Out) {
                                    ...
                                    if ($logsBetweenOutTimeAndRangeOut->isNotEmpty()) {
                                        ...
                                        $attendanceData['Present'] = 1;
                                        $attendanceData['TotalOT'] = abs($latestLog->diffInMinutes($outTime)); // Hitung TotalOT
                                    } else {
                                        ...
                                        if ($logsBetweenStartOutAndOutTime->isNotEmpty()) {
                                            ...
                                            $attendanceData['Time_Out'] = $latestLog->format('Y-m-d H:i:s');
                                            $attendanceData['Present'] = 1;
                                            $attendanceData['Time_OutShort'] = abs($latestLog->diffInMinutes($outTime));
                                            $attendanceData['TotalOT'] = 0; // Tidak ada OT jika Time_Out < Out_time
                                        } else {
                                ...
                                // Hitung TotalWorkHour dari tabel attend
                                if ($attendanceData['Time_In'] && $attendanceData['Time_Out'] && $attendanceData['Time_Break'] && $attendanceData['Time_Resume']) {
                                    $timeIn = Carbon::parse($attendanceData['Time_In'])->startOfMinute();
                                    $timeOut = Carbon::parse($attendanceData['Time_Out'])->startOfMinute();
                                    $timeBreak = Carbon::parse($attendanceData['Time_Break'])->startOfMinute();
                                    $timeResume = Carbon::parse($attendanceData['Time_Resume'])->startOfMinute();

                                    // TotalWorkHour = (Time_Out - Time_In) - (Time_Resume - Time_Break)
                                    $workDuration = $timeOut->diffInMinutes($timeIn);
                                    $breakDuration = $timeResume->diffInMinutes($timeBreak);
                                    $attendanceData['TotalWorkHour'] = max(0, $workDuration - $breakDuration);
                                }
                            } else {
                                $attendanceData['Time_In'] = null;
                                $attendanceData['Time_InShort'] = 0;
