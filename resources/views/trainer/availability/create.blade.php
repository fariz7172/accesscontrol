...
<head>
    ...
    <title>Soyal - Add Availability</title>
                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <span class="mr-2 d-none d-lg-inline text-gray-600 small">{{ Auth::guard('auth')->user()->username ?? Auth::guard('member')->user()->NAME ?? 'User' }}</span>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form method="POST" action="{{ route('trainer.availability.store') }}">
                                @csrf
                                @if(Auth::guard('auth')->user() && Auth::guard('auth')->user()->priv === 'superadmin')
                                <div class="form-group">
                                    <label for="PT_ID">Personal Trainer</label>
                                    <select id="PT_ID" name="PT_ID" class="form-control" required>
                                        <option value="">Select Trainer</option>
                                        @foreach(\App\Models\userProfileModel::where('user_type', '2')->select('ID', 'NAME')->get() as $trainer)
                                ...
                                @else
                                <input type="hidden" name="PT_ID" value="{{ $member->ID }}">
                                @endif
                                <div class="form-group">
