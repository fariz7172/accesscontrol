                        ...
                        <div class="col-sm-12 col-md-12 d-flex">
                            <form method="GET" action="{{ route('userProfiles.index') }}" class="d-flex">
                                ...
                                <div>
                                    ...
                                    <a href="#" class="btn btn-info btn-icon-split mt-4 ml-3 mb-3" id="generatePasswords">
                                        <span class="icon text-white-50"><i class="fas fa-key"></i></span>
                                        <span class="text">Generate Passwords</span>
                                    </a>
                                ...
                                <tbody>
                                    ...
                                    <tr id="userRow-{{ $userprofile->ID }}">
                                        ...
                                        <td style="text-align: center;">
                                            ...
                                            <img src="{{ $userprofile->photo }}" alt="Profile Photo" onerror="this.src='/path/to/default-image.jpg'" width="50" height="50">
                                            @else
                                            <img src="{{ asset('img/default.png') }}" alt="No Picture" width="50" height="50">
                                            @endif
                                        </td>
                                        <td style="width: 80px; ">
                                            <button id="deleteSelected" class="btn btn-danger btn-sm ml-3 btn-no-bg "> <i class="fas fa-fw fa-trash-alt mr-2" id="deleteSelected"></i></button>
                                            ...
                                            <button class="btn btn-info view-button btn-sm ml-3 btn-no-bg"
                                               ...
                                               data-picture="{{ $userprofile->photo ? $userprofile->photo : asset('img/default.png') }}"
...
<script>
    $(document).ready(function() {
        $('.view-button').on('click', function() {
            ...
            // Generate QR Code
            const qrContainer = document.getElementById("viewUserQRCode");
            qrContainer.innerHTML = ""; // Clear existing
            const downloadLink = document.getElementById('downloadUserQRCode');
            downloadLink.style.display = 'none';

            if (card && card != "0") {
                try {
                    // Convert Decimal to Hex
                    var hexCode = BigInt(card).toString(16).toUpperCase();
                    ...
                    // Update Download Link (with Composite Canvas)
                    setTimeout(() => {
                        const qrCanvas = qrContainer.querySelector('canvas');
                        const fileName = (username || 'User').replace(/\s+/g, '_') + '_QRCODE.jpg';
                        ...
                        if (qrCanvas) {
                            ...
                            ctx.fillText(username || 'User', width / 2, height - 10);
                    ...
                    qrContainer.innerHTML = "<p class='text-danger'>Invalid Card Data</p>";
                }
            } else {
                 qrContainer.innerHTML = "<p class='text-muted'>No RFID/Card Data</p>";
            }
        });
    });
</script>
...
<script>
    ...
    const syncMemberDreampost = '{{ $syncMemberDreampost }}';
    const syncMemberViewDreampos = '{{ $syncMemberViewDreampos }}';
    const sztimmyUserWeekzoneApiUrl = '{{ $sztimmyUserWeekzoneApiUrl }}';
    const INSERT_WEEKZONE_URL = '{{ route("userProfile.insertWeekzone") }}';
</script>

<script>
    window.sztimmyRegisterFaceApiUrl = sztimmyRegisterFaceApiUrl;
    window.sztimmyUserAccessApiUrl = sztimmyUserAccessApiUrl;
    window.sztimmyGetUserListApiUrl = sztimmyGetUserListApiUrl;
</script>
    ...
    $(document).ready(function() {
        ...
        $('#devicegroup_id').on('change', function() {
            ...
            $.ajax({
                ...
                method: 'GET',
                success: function(response) {
                    // Kosongkan container checkbox di dalam modal
                    $('#successModal .row .col-md-6').empty();

                    // Jika tidak ada data, tampilkan pesan
                    if (response.length === 0) {
                        $('#successModal .row .col-md-6:first').append('<p>No devices found for this group.</p>');
                        return;
                    }
                    ...
                    `;
                        $('#successModal .row .col-md-6:first').append(checkboxHtml);
                    });
                    ...
                    `;
                        $('#successModal .row .col-md-6:last').append(checkboxHtml);
                    });
                },
                error: function(xhr) {
                    console.error('Error fetching devices:', xhr);
                    $('#successModal .row .col-md-6').empty();
                    $('#successModal .row .col-md-6:first').append('<p>Error loading devices.</p>');
                }
            });
        });
...
<script>
$(document).ready(function() {
    // Handle Generate Passwords button click
    $('#generatePasswords').on('click', function(e) {
        e.preventDefault();

        Swal.fire({
            title: 'Are you sure?',
            text: 'This will generate new random passwords for all users. This action cannot be undone.',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Yes, generate passwords!',
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    ...
                    text: 'Please wait while passwords are being generated...',
                ...
                // Send AJAX request to generate passwords
                $.ajax({
                    url: '{{ route("userProfiles.generatePasswords") }}',
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success: function(response) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: response.message || 'Passwords for all users have been updated successfully.',
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: xhr.responseJSON?.message || 'Failed to generate passwords.',
                            confirmButtonText: 'OK'
                        });
                    }
                });
