    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('attendantSheet.index') }}" id="attendanceForm">
                ...
                <button type="submit" name="exportOffDay" value="1" class="btn btn-success">Export Off Day</button>
            </form>
        </div>
    </div>
                                    ...
                                    <td>
                                        ...
                                        <form class="update-remark-text-form" data-id="{{ $item->id }}"
                                            ...
                                            <select name="remark_select" class="form-control form-control-sm remark-select" data-id="{{ $item->id }}">
                                                ...
                                                <option value="Custom" {{ (!in_array($item->Remark, ['Holiday', null]) && $item->DutyProcessID == null) ? 'selected' : '' }}>Custom</option>
                                                <option value="Holiday" {{ $item->Remark === 'Holiday' ? 'selected disabled' : '' }}>Holiday</option>
                                                @foreach($leaveTypes as $leaveType)
                                                <option value="Leave|{{ $leaveType->Id }}"
                                                    {{ $item->DutyProcessID == $leaveType->Id ? 'selected' : '' }}>
                                                    {{ $leaveType->Name }}
                                                </option>
                                                @endforeach
                                            ...
                                            <input type="text" name="remark"
                                                class="form-control form-control-sm remark-text mt-1 {{ (!in_array($item->Remark, ['Holiday', null]) && $item->DutyProcessID == null) ? '' : 'd-none' }}"
                                                value="{{ $item->Remark ?? '' }}"
...
<script>
    $(document).ready(function() {
        // Toggle custom remark input
        $(document).on('change', '.remark-select', function() {
            let id = $(this).data('id');
            let value = $(this).val();
            let input = $(`input.remark-text[data-id="${id}"]`);

            if (value === 'Custom') {
                input.removeClass('d-none').focus();
            } else {
                input.addClass('d-none');
                // langsung submit kalau pilih Holiday / Leave / N/A
                let form = $(this).closest('form');
                form.submit();
            }
        });

        // Submit remark via AJAX (select & custom input)
        $(document).on('change', '.remark-text', function() {
            let form = $(this).closest('form');
            form.submit();
        });

        // Prevent default form submit, use AJAX
        $('.update-remark-text-form').on('submit', function(e) {
            e.preventDefault();

            let form = $(this);
            let url = form.attr('action');
            let id = form.data('id');
            let remarkSelect = form.find('.remark-select').val();
            let remarkInput = form.find('.remark-text').val();
            ...
            $.ajax({
                ...
                data: {
                    ...
                    _method: 'PATCH',
                    id: id,
                    remark_data: remarkSelect === 'Custom' ? remarkInput : remarkSelect
                },
                success: function(response) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
                        text: response.message || 'Remark updated!',
                        willClose: () => {
                            window.location.href = response.redirect || '{{ route("attendantSheet.index") }}';
                        }
                    });
                },
                error: function(xhr) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: xhr.responseJSON?.message || 'Failed to update remark'
                    });
                }
            });
        });
